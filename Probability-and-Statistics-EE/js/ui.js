/* Shared UI toggles for the probability and statistics problem set:
   - theme:  light / dark   (persisted in localStorage as "prob-theme")
   - language: en / zh       (persisted in localStorage as "prob-lang")
   - page scale: +/- zoom    (persisted in localStorage as "prob-scale")
   - full-screen view        (intent persisted in localStorage as "prob-fs")
...
(function () {
  ...
  function readScale() {
    var v;
    try { v = parseFloat(localStorage.getItem("prob-scale")); } catch (e) {}
  ...
  function setScale(next) {
    ...
    try { localStorage.setItem("prob-scale", String(scale)); } catch (e) {}
    applyScale();
  }
  ...
  /* ---- full-screen ----
     ...
     gesture). So we persist the user's *intent* ("prob-fs") and, on the next
  ...
  function stickyOffset(topbar) {
    var breadcrumb = document.querySelector(".breadcrumb");
    var crumbHeight = breadcrumb && !topbar.contains(breadcrumb) ? breadcrumb.getBoundingClientRect().height : 0;
    return headerHeight(topbar) + crumbHeight;
  }
  ...
  function scrollToHeading(h, topbar) {
    ...
    // correctly at any zoom level; the offset clears sticky navigation.
    var y = window.scrollY + h.getBoundingClientRect().top - stickyOffset(topbar) - 12;
  ...
  function buildToc(controls, topbar) {
    ...
    function updateActive() {
      ticking = false;
      var off = stickyOffset(topbar) + 14, cur = null;
  ...
  /* ---- lightweight syntax highlighting ----
     Section pages keep short, editable snippets in <pre><code>. This highlighter
     colors common Python, JavaScript, C/C++, and HTML tokens without changing
     the text returned by innerText, so Run / Copy buttons still read the source. */
  function esc(s) {
    return String(s).replace(/[&<>"']/g, function (ch) {
      return { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[ch];
    });
  }
  function cls(name, text) { return '<span class="tok-' + name + '">' + esc(text) + '</span>'; }
  function langOf(code) {
    var m = (code.className || "").match(/(?:^|\s)language-([a-z0-9+#-]+)/i);
    return m ? m[1].toLowerCase() : "";
  }
  function keywordSet(words) {
    var out = {};
    words.split(/\s+/).forEach(function (w) { if (w) out[w] = true; });
    return out;
  }
  var KEYWORDS = {
    python: keywordSet("and as assert break class continue def del elif else except False finally for from global if import in is lambda None nonlocal not or pass raise return True try while with yield print range len int float str list dict set tuple"),
    javascript: keywordSet("async await break case catch class const continue debugger default delete do else export extends false finally for from function if import in instanceof let new null of return static super switch this throw true try typeof undefined var void while with yield"),
    js: null,
    c: keywordSet("auto break case char const continue default do double else enum extern float for goto if inline int long register restrict return short signed sizeof static struct switch typedef union unsigned void volatile while"),
    cpp: keywordSet("alignas alignof and asm auto bool break case catch char class const constexpr continue decltype default delete do double else enum explicit export extern false float for friend goto if inline int long namespace new noexcept nullptr operator private protected public register return short signed sizeof static_assert static_cast struct switch template this throw true try typedef typename union unsigned using virtual void volatile while")
  };
  KEYWORDS.js = KEYWORDS.javascript;
  KEYWORDS["c++"] = KEYWORDS.cpp;
  KEYWORDS.cc = KEYWORDS.cpp;
  KEYWORDS.html = keywordSet("doctype html head body title meta link script style div span main header footer section article nav p a img figure figcaption table tr th td ul ol li pre code");

  function highlightPlain(text, lang) {
    var keys = KEYWORDS[lang] || {};
    var out = "", re = /([A-Za-z_$][\w$]*)(\s*\()?|(\b\d+(?:\.\d+)?(?:e[+-]?\d+)?\b)|([+\-*\/%=!<>|&^~?:]+)/ig, last = 0, m;
    while ((m = re.exec(text))) {
      out += esc(text.slice(last, m.index));
      if (m[1]) {
        if (keys[m[1]]) out += cls("keyword", m[1]) + esc(m[2] || "");
        else if (m[2]) out += cls("function", m[1]) + esc(m[2]);
        else out += esc(m[1]);
      } else if (m[3]) out += cls("number", m[3]);
      else out += cls("operator", m[4]);
      last = re.lastIndex;
    }
  ...
  function highlightCodeText(text, lang) {
    ...
    while (i < n) {
      if ((lang === "python" || lang === "py") && text[i] === "#") {
        var pyEnd = text.indexOf("\n", i);
        if (pyEnd < 0) pyEnd = n;
        out += cls("comment", text.slice(i, pyEnd));
        i = pyEnd;
      } else if (text[i] === "/" && text[i + 1] === "/") {
        var lineEnd = text.indexOf("\n", i);
        if (lineEnd < 0) lineEnd = n;
        out += cls("comment", text.slice(i, lineEnd));
        i = lineEnd;
      } else if (text[i] === "/" && text[i + 1] === "*") {
        var blockEnd = text.indexOf("*/", i + 2);
        blockEnd = blockEnd < 0 ? n : blockEnd + 2;
        out += cls("comment", text.slice(i, blockEnd));
        i = blockEnd;
      } else if (text[i] === '"' || text[i] === "'" || text[i] === "`") {
        var q = text[i], j = i + 1;
        while (j < n) {
          if (text[j] === "\\") j += 2;
          else if (text[j] === q) { j++; break; }
          else j++;
        }
        out += cls("string", text.slice(i, j));
        i = j;
      } else {
        var next = n, marks = ['"', "'", "`", "//", "/*"];
        if (lang === "python" || lang === "py") marks.push("#");
        marks.forEach(function (mark) {
          var p = text.indexOf(mark, i + (mark.length > 1 ? 1 : 0));
          if (p >= 0 && p < next) next = p;
        });
        out += highlightPlain(text.slice(i, next), lang === "py" ? "python" : lang);
        i = next;
      }
    }
    return out;
  ...
  function highlightBlock(code) {
    ...
    if (!lang || code.getAttribute("data-highlighted") === "true") return;
    code.setAttribute("data-source", code.innerText);
    code.innerHTML = highlightCodeText(code.innerText, lang);
    code.setAttribute("data-highlighted", "true");
    if (code.isContentEditable) {
      code.addEventListener("input", function () {
        code.setAttribute("data-highlighted", "dirty");
      });
  ...
  function highlightCodeBlocks() {
    Array.prototype.forEach.call(document.querySelectorAll("pre code[class*='language-']"), highlightBlock);
  }
  window.probHighlightCodeBlocks = highlightCodeBlocks;
  ...
  function init() {
    ...
    if (themeBtn) {
      ...
      themeBtn.addEventListener("click", function () {
        ...
        try { localStorage.setItem("prob-theme", next); } catch (e) {}
        themeBtn.textContent = labelTheme();
      });
    }
    ...
    if (langBtn) {
      ...
      langBtn.addEventListener("click", function () {
        ...
        try { localStorage.setItem("prob-lang", next); } catch (e) {}
        langBtn.textContent = labelLang();
      });
    }
